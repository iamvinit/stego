...
def changeSizeOfBlockMul(sizemul):
	global SIZEMUL
	SIZEMUL = sizemul
...
def generateKeyFromString(sizeOfData, str):
	...
	for cbyte in str_bytes:
		size_rep = size_rep + ((cbyte >> 2) * SIZEMUL)
...
def encrypt(data, strKey):
	...
	for blockConfig in key:
		sizeOfBlock = (blockConfig >> 2 ) * SIZEMUL
		
		if blockConfig == key[-1]:
			sizeOfBlock = sizeOfData - sizeOfDataDone
